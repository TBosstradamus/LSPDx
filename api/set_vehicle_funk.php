...
if (!$data || !isset($data['vehicleId']) || !isset($data['funk'])) {
...
try {
    ...
    $success = $vehicleModel->updateFunk($data['vehicleId'], $data['funk']);

    if ($success) {
        echo json_encode(['success' => true]);
    } else {
        http_response_code(500);
        echo json_encode(['error' => 'Failed to update vehicle funk.']);
    }
} catch (Exception $e) {
    http_response_code(500);
