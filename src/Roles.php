...
class Roles {
    ...
    /**
     * Gets all permissions for a specific user by their officer ID.
     * @param int $officerId
    ...
    public function getPermissionsForUser($officerId) {
        try {
            ...
            // fetchAll with PDO::FETCH_COLUMN returns a flat array of permission names
    ...
    public function getRolesForUser($officerId) {
        try {
            $sql = "SELECT r.* FROM roles r JOIN user_roles ur ON r.id = ur.role_id WHERE ur.officer_id = :officer_id";
            $stmt = $this->db->prepare($sql);
            $stmt->execute([':officer_id' => $officerId]);
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
    ...
    public function updateUserRoles($officerId, $roleIds) {
        ...
        try {
            ...
            $deleteStmt = $this->db->prepare("DELETE FROM user_roles WHERE officer_id = :officer_id");
            $deleteStmt->execute([':officer_id' => $officerId]);

            // Then, insert the new roles
            if (!empty($roleIds)) {
                $insertSql = "INSERT INTO user_roles (officer_id, role_id) VALUES (:officer_id, :role_id)";
                $insertStmt = $this->db->prepare($insertSql);
                foreach ($roleIds as $roleId) {
                    $insertStmt->execute([':officer_id' => $officerId, ':role_id' => $roleId]);
                }
            }
            $this->db->commit();
            return true;
